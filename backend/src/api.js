...
app.post("/api/juegos", async (req, res) => {
  try {
    ...
    if (!req.body.nombre) {
      return res
        .status(400)
        .json({ error: "El nombre del juego es obligatorio" });
    }

    if (!req.body.anio) {
      return res.status(400).json({ error: "El año del juego es obligatorio" });
    }

    if (!req.body.descripcion) {
      return res
        .status(400)
        .json({ error: "La descripción del juego es obligatoria" });
    }

    if (!req.body.desarrolladora) {
      return res
        .status(400)
        .json({ error: "La desarrolladora es obligatoria" });
    }
    ...
    if (req.body.nombre.length > 100) {
      return res
        .status(400)
        .json({ error: "El nombre del juego no puede exceder 100 caracteres" });
    }

    if (req.body.descripcion.length > 200) {
      return res
        .status(400)
        .json({ error: "La descripción no puede exceder 200 caracteres" });
    }

    if (req.body.genero.length > 100) {
      return res
        .status(400)
        .json({ error: "El género no puede exceder 100 caracteres" });
    }

    if (req.body.url_imagen && req.body.url_imagen.length > 200) {
      return res
        .status(400)
        .json({ error: "La URL de la imagen no puede exceder 200 caracteres" });
    }
    ...
    if (error.code) {
      switch (error.code) {
        case "23502": // NOT NULL violation
          return res
            .status(400)
            .json({ error: "Faltan campos obligatorios en la base de datos" });
        case "23505": // Unique violation
          return res
            .status(400)
            .json({ error: "Ya existe un juego con ese nombre" });
        case "23503": // Foreign key violation
          return res
            .status(400)
            .json({ error: "La desarrolladora seleccionada no existe" });
        case "22001": // String data right truncation
          return res.status(400).json({
            error: "Uno de los campos excede el límite de caracteres permitido",
          });
        default:
          return res
            .status(500)
            .json({ error: `Error de base de datos: ${error.message}` });
      }
    }
...
app.put("/api/juegos/:id", async (req, res) => {
  try {
    ...
    if (!req.body.anio) {
      return res.status(400).json({ error: "El año obligatorio" });
    }
    ...
    console.log(
      "PUT /api/juegos/:id - Validaciones pasadas, llamando updateJuego"
    );
