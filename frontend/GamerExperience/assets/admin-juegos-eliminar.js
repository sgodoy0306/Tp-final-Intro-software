...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!infoJuegoEliminar || !mensaje || !juegoSelector || !juegosLista || !btnEliminar || !checkboxConfirmar) return;
    ...
    function obtenerNombreDesarrolladora(desarrolladoraId) {
        if (!desarrolladoraId || !desarrolladorasData.length) return "Sin desarrolladora";
        
        const desarrolladora = desarrolladorasData.find(dev => dev.id === desarrolladoraId);
    ...
    function mostrarListaJuegos(juegos, totalResultados = null) {
        ...
        if (totalResultados && totalResultados > juegos.length) {
            ...
            infoDiv.className = "p-2 text-center text-amber-300 text-xs border-b border-[#333] mb-2";
        ...
        juegos.forEach(juego => {
            const div = document.createElement("div");
            div.className = "p-3 bg-[#232222] rounded border border-[#333] cursor-pointer hover:bg-[#2a2929] transition";

            // Resaltar el término buscado en el nombre
            let nombreConResaltado = juego.nombre;
            if (termino.length > 0) {
                const regex = new RegExp(`(${termino})`, 'gi');
                nombreConResaltado = juego.nombre.replace(regex, '<span class="bg-red-500 text-white px-1 rounded">$1</span>');
            }

            div.innerHTML = `
                <div class="font-bold text-amber-50">${nombreConResaltado}</div>
                <div class="text-sm text-amber-300">${juego.anio} - ${juego.genero}</div>
                <div class="text-xs text-gray-400">${juego.desarrolladora_nombre || 'Sin desarrolladora'}</div>
            `;
    ...
    async function cargarDatosJuego(juego) {
        try {
            ...
            document.getElementById("info-nombre").textContent = juegoCompleto.nombre || "-";
            document.getElementById("info-anio").textContent = juegoCompleto.anio || "-";
            document.getElementById("info-desarrolladora").textContent = obtenerNombreDesarrolladora(juegoCompleto.desarrolladora);
            document.getElementById("info-genero").textContent = juegoCompleto.genero || "-";
            document.getElementById("info-descripcion").textContent = juegoCompleto.descripcion || "-";

            // Mostrar consolas
            if (juegoCompleto.consolas && Array.isArray(juegoCompleto.consolas)) {
                const consolasNombres = juegoCompleto.consolas.map(consola => consola.nombre);
                document.getElementById("info-consolas").textContent = consolasNombres.join(", ") || "-";
            } else {
