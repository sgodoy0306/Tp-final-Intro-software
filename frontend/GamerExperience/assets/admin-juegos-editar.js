...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function mostrarListaJuegos(juegos, totalResultados = null) {
        ...
        juegos.forEach(juego => {
            ...
            if (termino.length > 0) {
                ...
                nombreConResaltado = juego.nombre.replace(regex, '<span class="bg-amber-500 text-[#131212] px-1 rounded">$1</span>');
            }
    ...
    async function cargarDatosJuego(juego) {
        try {
            ...
            document.getElementById("nombre-juego").value = juegoCompleto.nombre || "";
            document.getElementById("anio-juego").value = juegoCompleto.anio || "";
            document.getElementById("descripcion-juego").value = juegoCompleto.descripcion || "";
            document.getElementById("portada-juego").value = juegoCompleto.url_imagen || "";

            // Para la desarrolladora, usar el campo desarrolladora (que es el ID)
            const desarrolladoraId = juegoCompleto.desarrolladora;
            if (desarrolladoraId) {
                const desarrolladoraSelect = document.getElementById("desarrolladora-juego");
    ...
    async function populateSelect(url, selectId, textKey = "nombre", valueKey = "id") {
        try {
            ...
            data.forEach(item => {
                ...
                option.textContent = item[textKey] || item[textKey.charAt(0).toUpperCase() + textKey.slice(1)] || item.Nombre || item.nombre;
    ...
    function actualizarConsolasSeleccionadas() {
        const checkboxes = document.querySelectorAll('input[name="consolas"]:checked');
    ...
    form.addEventListener("submit", async function (e) {
        ...
        if (!datosJuego.nombre || datosJuego.nombre.length < 2) {
            mensaje.textContent = "El nombre del juego debe tener al menos 2 caracteres";
            mensaje.className = "font-pixel text-center mt-4 text-red-500";
            return;
        }
        ...
        if (isNaN(datosJuego.anio) || datosJuego.anio < 1950 || datosJuego.anio > new Date().getFullYear()) {
