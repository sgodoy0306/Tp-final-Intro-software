...
function poblarFiltros() {
    ...
    desarrolladoraFilter.innerHTML = '<option value="">Todas las desarrolladoras</option>';
    ...
    juegosFiltrados = todosLosJuegos.filter(juego => {
        ...
        if (consolaId && juego.consolas) {
            ...
            if (Array.isArray(juego.consolas)) {
                consolaCoincide = juego.consolas.some(consola => 
                    (consola.id || consola) == consolaId
                );
            } else {
        ...
        return nombreCoincide && desarrolladoraCoincide && generoCoincide && consolaCoincide;
    });
...
function crearTarjetaJuego(juego) {
    ...
    card.className = 'bg-[#181612]/80 backdrop-blur-md rounded-2xl border border-[#23201c] shadow-2xl overflow-hidden hover:scale-105 transition-all duration-300 hover:border-yellow-400 cursor-pointer';
    ...
    const desarrolladora = todasLasDesarrolladoras.find(dev => 
        dev.id === (juego.desarrolladora || juego.Desarrolladora)
    );

    // Obtener información de las consolas
    let consolasTexto = 'No disponible';
    if (juego.consolas && Array.isArray(juego.consolas)) {
        consolasTexto = juego.consolas.map(consola => 
    ...
    card.innerHTML = `
        <div class="relative">
            <img 
                src="${juego.url_imagen || juego.URL_IMAGEN || 'https://via.placeholder.com/300x200?text=No+Image'}" 
        ...
        <div class="p-4">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex items-center">
                    ...
                    <span class="text-gray-300 font-pixel">${juego.genero || juego.Genero || 'N/A'}</span>
                </div>
                
                <div class="flex items-center">
                    <span class="text-green-400 font-pixel w-20">Dev:</span>
                    <span class="text-gray-300 font-pixel truncate">${desarrolladora ? (desarrolladora.nombre || desarrolladora.Nombre) : 'N/A'}</span>
                </div>
...
function toggleFiltros() {
    ...
    if (isHidden) {
        ...
        toggleFiltersBtn.classList.remove('bg-green-600', 'hover:bg-green-700', 'border-green-500');
        toggleFiltersBtn.classList.add('bg-red-600', 'hover:bg-red-700', 'border-red-500');
    } else {
        filtersContainer.classList.remove('block');
        filtersContainer.classList.add('hidden');
        toggleFiltersBtn.textContent = 'MOSTRAR FILTROS';
        toggleFiltersBtn.classList.remove('bg-red-600', 'hover:bg-red-700', 'border-red-500');
        toggleFiltersBtn.classList.add('bg-green-600', 'hover:bg-green-700', 'border-green-500');
    }
}
...
function mostrarDetallesJuego(juego) {
    ...
    document.getElementById('modalTitulo').textContent = juego.nombre || juego.Nombre || 'Sin título';
    document.getElementById('modalImagen').src = juego.url_imagen || juego.URL_IMAGEN || 'https://via.placeholder.com/400x300?text=No+Image';
    document.getElementById('modalImagen').alt = juego.nombre || juego.Nombre || '';
    document.getElementById('modalAnio').textContent = juego.anio || juego.Anio || 'No disponible';
    document.getElementById('modalGenero').textContent = juego.genero || juego.Genero || 'No disponible';
    document.getElementById('modalDescripcion').textContent = juego.descripcion || juego.Descripcion || 'No hay descripción disponible';

    // Llenar información de la desarrolladora
    if (desarrolladora) {
        document.getElementById('modalDesarrolladoraNombre').textContent = desarrolladora.nombre || desarrolladora.Nombre || 'No disponible';
        document.getElementById('modalDesarrolladoraFundacion').textContent = desarrolladora.fundacion || desarrolladora.Fundacion || 'No disponible';
        document.getElementById('modalDesarrolladoraPais').textContent = desarrolladora.pais || desarrolladora.Pais || 'No disponible';
    } else {
        document.getElementById('modalDesarrolladoraNombre').textContent = 'No disponible';
        document.getElementById('modalDesarrolladoraFundacion').textContent = 'No disponible';
        document.getElementById('modalDesarrolladoraPais').textContent = 'No disponible';
    }
    ...
    if (juego.consolas && Array.isArray(juego.consolas) && juego.consolas.length > 0) {
        juego.consolas.forEach(consola => {
            // Buscar información completa de la consola
            const consolaCompleta = todasLasConsolas.find(c => 
                c.id === (consola.id || consola) || 
                (c.nombre || c.Nombre) === (consola.nombre || consola.Nombre || consola)
            );

            const consolaElement = document.createElement('div');
            consolaElement.className = 'bg-black/50 p-3 rounded border border-gray-600 text-center cursor-pointer hover:bg-gray-700/50 hover:border-yellow-400 transition-all duration-200 transform hover:scale-105';
        ...
        noConsolasElement.className = 'col-span-2 bg-black/50 p-3 rounded border border-gray-600 text-center';
